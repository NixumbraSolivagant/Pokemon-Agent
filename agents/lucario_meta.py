...
USE_SEARCH = True
SEARCH_TIME_BUDGET = 0.65
SEARCH_MAX_CANDIDATES = 6
SEARCH_BELIEF_WORLDS = 4
SEARCH_RISK_PENALTY = 0.28
SEARCH_MARGIN = 180.0
BEAM_WIDTH = 3
MCTS_ITERATIONS = 15
OPPONENT_DECK_MODELS = {}
...
class C:
    ...
    JUDGE, BATTLE_CAGE = 1213, 1264
    LUMIOSE_CITY, LILLIES_PEARL, LEGACY_ENERGY = 1267, 1172, 12
    GREAT_TUSK, DWEBBLE, CRUSTLE = 58, 344, 345
    HYDRAPPLE_EX, DURALUDON, ARCHALUDON_EX, CINDERACE = 150, 169, 190, 666

MEGA_BRAVE = 983
LOW_DECK_COUNT = 10
MILL_DECK_RESERVE = 14

def read_deck() -> list[int]:
    candidates = [Path("/kaggle_simulations/agent/deck.csv"), Path("deck.csv")]
    module_file = globals().get("__file__")
    if module_file:
        candidates.insert(0, Path(module_file).resolve().parent / "deck.csv")
    path = next((candidate for candidate in candidates if candidate.exists()), None)
    if path is None:
        return list(DECK)
    return [int(line) for line in path.read_text(encoding="utf-8").splitlines() if line.strip()][:60]
...
class AdvancedPolicy:
    ...
    def _opponent_is_mill_deck(self) -> bool: return self._opponent_has({C.GREAT_TUSK})
    def _opponent_is_hand_scaler(self) -> bool: return self._opponent_has({245, 741, 742, 743})
    def _opponent_uses_damage_counters(self) -> bool: return self._opponent_has({119, 120, 121})
    ...
    def _score_play(self, option) -> float:
        ...
        conserve_limit = MILL_DECK_RESERVE if self._opponent_is_mill_deck() else LOW_DECK_COUNT
        if self.me.deckCount <= conserve_limit and card.id in {
            C.DUSK_BALL, C.FIGHTING_GONG, C.POKE_PAD, C.CARMINE, C.LILLIE_DETERMINATION,
        }:
            return -1
        ...
        if card.id == C.CARMINE:
            ...
            return -1 if self.me.deckCount <= conserve_limit else 3000
        if card.id == C.LILLIE_DETERMINATION:
            return -1 if self.me.deckCount <= conserve_limit else 3100
        if card.id == C.JUDGE:
            if self.me.deckCount <= LOW_DECK_COUNT: return -1
            if self._opponent_is_hand_scaler(): return 3900
            if self.opponent.handCount >= 7: return 3500
            if self.me.handCount <= 3 and self.opponent.handCount >= 5: return 3150
            return -1
        ...
        if card.id == C.BATTLE_CAGE: return 3600 if self._opponent_uses_damage_counters() else (800 if self.stadium_id else -1)
        return 10000
...
def evaluate_state(obs):
    ...
    is_crustle = any(p is not None and p.id in {C.DWEBBLE, C.CRUSTLE} for p in [op.active[0] if op.active else None] + list(op.bench))
    is_mill = any(p is not None and p.id == C.GREAT_TUSK for p in [op.active[0] if op.active else None] + list(op.bench))
    ...
    for p in [op.active[0] if op.active else None] + list(op.bench):
        ...
        elif p.id == 345: op_dmg = 120 if assumed_energies >= 3 else 0
        elif p.id == 648: op_dmg = 180 if assumed_energies >= 2 else 0
        elif p.id == 381: op_dmg = 260 if assumed_energies >= 2 else 100 if assumed_energies >= 1 else 0
        elif p.id == 743: op_dmg = getattr(op, "handCount", 0) * 20 if assumed_energies >= 1 else 0
        elif p.id == 1031: op_dmg = 210 if assumed_energies >= 3 else 120 if assumed_energies >= 1 else 0
        elif p.id == C.ARCHALUDON_EX: op_dmg = 220 if assumed_energies >= 3 else 0
        elif p.id == C.DURALUDON: op_dmg = 80 + max(0, card_table[p.id].hp - p.hp) if assumed_energies >= 2 else 30 if assumed_energies >= 1 else 0
        elif p.id == C.CINDERACE: op_dmg = 50 if assumed_energies >= 1 else 0; val -= 600.0
        elif p.id == C.HYDRAPPLE_EX:
            attached = sum(len(q.energies) for q in [op.active[0] if op.active else None] + list(op.bench) if q is not None)
            op_dmg = 30 + attached * 30 if assumed_energies >= 2 else 0
        elif p.id == C.GREAT_TUSK: op_dmg = 160 if assumed_energies >= 4 else 0
    ...
    if is_mill:
        projected_deck = deck_c - 5
        if projected_deck <= 0: val -= 50000.0
        elif projected_deck <= 4: val -= (5 - projected_deck) * 5000.0
...
def _attached_ids(pokemon):
    if pokemon is None:
        return []
    cards = list(getattr(pokemon, "energyCards", []) or []) + list(getattr(pokemon, "tools", []) or [])
    return [int(getattr(card, "id", 0) or 0) for card in cards if card is not None]


def _visible_ids(player, include_hand=False):
    result = []
    zones = [getattr(player, "active", []), getattr(player, "bench", []), getattr(player, "discard", [])]
    if include_hand:
        zones.append(getattr(player, "hand", []))
    for zone in zones:
        for card in zone or []:
            if card is None:
                continue
            result.append(int(getattr(card, "id", 0) or 0))
            result.extend(_attached_ids(card))
    return [card_id for card_id in result if card_id > 0]


def _observation_seed(obs, salt=0):
    current = obs.current
    seed = int(getattr(current, "turn", 0) or 0) * 1000003 + int(salt)
    for player in current.players:
        for card_id in _visible_ids(player, include_hand=False):
            seed = (seed * 1009 + card_id) & 0x7FFFFFFF
    return seed


def _partition(deck, counts, visible, rng, fallback):
    available = Counter(deck)
    for card_id in visible:
        if available[card_id] > 0:
            available[card_id] -= 1
    pool = [card_id for card_id, count in available.items() for _ in range(max(0, count))]
    rng.shuffle(pool)
...
def _rank_opponent_models(opponent):
    models = {str(name): list(deck) for name, deck in OPPONENT_DECK_MODELS.items() if len(deck) == 60}
    if not models:
        return [("unknown", list(my_deck))]
    visible = set(_visible_ids(opponent, include_hand=False))
    visible_pokemon = {
        card_id for card_id in visible
        if card_id in card_table and card_table[card_id].cardType == CardType.POKEMON
    }

    def model_score(item):
        _, deck = item
        model_cards = set(deck)
        model_pokemon = {
            card_id for card_id in model_cards
            if card_id in card_table and card_table[card_id].cardType == CardType.POKEMON
        }
        missing_pokemon = len(visible_pokemon - model_pokemon)
        pokemon_overlap = len(visible_pokemon & model_pokemon)
        missing_visible = len(visible - model_cards)
        total_overlap = len(visible & model_cards)
        return (-missing_pokemon, pokemon_overlap, -missing_visible, total_overlap)
    ...
    if not ranked or not visible:
        return ranked
    best_score = model_score(ranked[0])
    return [item for item in ranked if model_score(item) == best_score]


def _sample_world(obs, world_index):
    state = obs.current
    me = state.players[state.yourIndex]
    opponent = state.players[1 - state.yourIndex]
    rng = random.Random(_observation_seed(obs, world_index))
    ranked = _rank_opponent_models(opponent)
    _, opponent_model = ranked[world_index % len(ranked)]
    own_deck, own_prize = _partition(
        my_deck,
    ...
    opponent_deck, opponent_prize, opponent_hand = _partition(
        opponent_model,
        (opponent.deckCount, len(opponent.prize), opponent.handCount),
        _visible_ids(opponent, include_hand=False),
        rng,
        C.RIOLU,
    )
...
def _search_budget(obs_dict, obs):
    remaining = float(obs_dict.get("remainingOverageTime", 600.0) or 0.0)
    if remaining < 45.0:
        return 0.0
    me = obs.current.players[obs.current.yourIndex]
    late_game = len(me.prize) <= 2
    multiplier = 1.45 if late_game else 1.0
    if remaining < 120.0:
        multiplier *= 0.45
    return min(1.20, SEARCH_TIME_BUDGET * multiplier)


def _critical_candidates(obs, ordered):
    critical_types = {OptionType.ATTACK, OptionType.EVOLVE, OptionType.RETREAT, OptionType.ATTACH}
    result = []
    for index in ordered:
        option = obs.select.option[index]
        if option.type in critical_types:
            result.append(index)
            continue
        if option.type == OptionType.PLAY:
            card = get_card(obs, AreaType.HAND, option.index, obs.current.yourIndex)
            if card is not None and card.id in {
                C.BOSS_ORDERS, C.SWITCH, C.DUSK_BALL, C.FIGHTING_GONG,
                C.CARMINE, C.LILLIE_DETERMINATION, C.JUDGE,
            }:
                result.append(index)
    if len(result) < 2:
        return ordered[:SEARCH_MAX_CANDIDATES]
    return result[:SEARCH_MAX_CANDIDATES]

def SEARCH_ALGO(obs_dict, obs):
    if not (_SEARCH_OK and USE_SEARCH): return None
    select = obs.select
    if select is None or select.context != SelectContext.MAIN or not getattr(obs, "search_begin_input", None): return None
    t0 = time.time()
    budget = _search_budget(obs_dict, obs)
    if budget <= 0.0: return None

    base_order = AdvancedPolicy(obs).choose()
    candidates = _critical_candidates(obs, base_order)
    if not candidates: return None
    if len(candidates) == 1: return [candidates[0]] + [i for i in base_order if i != candidates[0]]

    values = {action: [] for action in candidates}
    try:
        deadline = t0 + budget
        for world_index in range(max(1, SEARCH_BELIEF_WORLDS)):
            if time.time() >= deadline:
                break
            own_deck, own_prize, opponent_deck, opponent_prize, opponent_hand = _sample_world(obs, world_index)
            root = search_begin(
                obs,
                own_deck,
                own_prize,
            ...
            try:
                for action in candidates:
                    if time.time() >= deadline:
                        break
                    result = search_step(root.searchId, [action])
                    final_obs = rollout_turn(result.searchId, result.observation, obs.current.yourIndex)
                    values[action].append(evaluate_state(final_obs))
            finally:
                search_end()

        aggregates = {}
        for action, samples in values.items():
            if not samples:
                continue
            mean = sum(samples) / len(samples)
            variance = sum((value - mean) ** 2 for value in samples) / len(samples)
            aggregates[action] = mean - SEARCH_RISK_PENALTY * math.sqrt(variance)
        if not aggregates:
            return None
        heuristic = base_order[0]
        best_action = max(aggregates, key=aggregates.get)
        if heuristic in aggregates and aggregates[best_action] < aggregates[heuristic] + SEARCH_MARGIN:
            best_action = heuristic
        return [best_action] + [index for index in base_order if index != best_action]
